...
@Configurable
@TeleOp
public class FieldTeleOpNormal extends OpMode {
    ...
    // This converts turnRate (-1.0 to 1.0) to predicted angular velocity (deg/sec)
    ...
    public void loop() {
        ...
        if (!automatedDrive) {
            ...
            // Normal turning - direct joystick input (no exponential curve)
            ...
            if (!slowMode) follower.setTeleOpDrive(
                ...
                //This is how it looks with slowMode on
                    ...
                    (turnRate + strafeCorrection) * slowModeMultiplier,
                    true // Robot Centric
            );
        }
/*
 ...
 */
        ...
        if (turretMode == TurretMode.FIELD_RELATIVE) {
            ...
            if (Math.abs(filteredActualAngularVelocity) > MIN_ACTUAL_VELOCITY_THRESHOLD) {
                ...
                double turnRate = -gamepad1.right_stick_x; // Normal turn rate (no exponential)
                double predictedRobotAngularVelocity = turnRate * maxRobotAngularVelocity; // deg/sec
        ...
        telemetryM.debug("turnRate", -gamepad1.right_stick_x);
        ...
        if (turretMode == TurretMode.FIELD_RELATIVE) {
            ...
            double turnRate = -gamepad1.right_stick_x; // Normal turn rate (no exponential)
            double predictedRobotAngularVelocity = turnRate * maxRobotAngularVelocity;
            ...
            telemetry.addData("Turn Rate", "%.3f", turnRate);
